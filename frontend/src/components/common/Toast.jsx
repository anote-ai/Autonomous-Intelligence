import React, { useEffect } from "react";
import { useTheme } from "../../hooks/useTheme";

const Toast = ({ message, type = "error", onClose, duration = 5000 }) => {
  const { theme } = useTheme();

  useEffect(() => {
    if (duration && onClose) {
      const timer = setTimeout(() => {
        onClose();
      }, duration);

      return () => clearTimeout(timer);
    }
  }, [duration, onClose]);

  if (!message) return null;

  const typeStyles = {
    error: {
      light: "bg-red-50 text-red-800 border-red-200",
      dark: "bg-red-900/90 text-red-100 border-red-700",
      icon: (
        <path
          fillRule="evenodd"
          d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z"
          clipRule="evenodd"
        />
      ),
    },
    success: {
      light: "bg-green-50 text-green-800 border-green-200",
      dark: "bg-green-900/90 text-green-100 border-green-700",
      icon: (
        <path
          fillRule="evenodd"
          d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
          clipRule="evenodd"
        />
      ),
    },
    warning: {
      light: "bg-yellow-50 text-yellow-800 border-yellow-200",
      dark: "bg-yellow-900/90 text-yellow-100 border-yellow-700",
      icon: (
        <path
          fillRule="evenodd"
          d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z"
          clipRule="evenodd"
        />
      ),
    },
    info: {
      light: "bg-blue-50 text-blue-800 border-blue-200",
      dark: "bg-blue-900/90 text-blue-100 border-blue-700",
      icon: (
        <path
          fillRule="evenodd"
          d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z"
          clipRule="evenodd"
        />
      ),
    },
  };

  const styles = typeStyles[type] || typeStyles.error;

  return (
    <div className="fixed top-16 right-4 z-50 max-w-md animate-slide-in">
      <div
        className={`flex items-start gap-3 p-4 rounded-lg shadow-lg border ${
          theme === "dark" ? styles.dark : styles.light
        }`}
      >
        <svg
          className="w-5 h-5 shrink-0 mt-0.5"
          fill="currentColor"
          viewBox="0 0 20 20"
        >
          {styles.icon}
        </svg>
        <div className="flex-1">
          <p className="font-medium text-sm">{message}</p>
        </div>
        {onClose && (
          <button
            onClick={onClose}
            className="shrink-0 hover:opacity-70 transition-opacity"
            aria-label="Close notification"
          >
            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
              <path
                fillRule="evenodd"
                d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
                clipRule="evenodd"
              />
            </svg>
          </button>
        )}
      </div>
    </div>
  );
};

export default Toast;
